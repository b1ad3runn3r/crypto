Здесь и далее используется AES с длиной ключа 128 бит.

## 1. Реализовать функцию на вашем языке программирования со следующим интерфейсом:

### `void SetKey(byte[] key)` - установка ключа шифрования\расшифрования, где

`key` - байтовое представление ключа блочного шифра

### `void SetMode(string mode)` - указание режима шифрования, где

`mode` - режим шифрования, допустимые значения ECB, CBC, CFB, OFB, CTR. Может быть задан через `Enum` на c#, или через константы.

### `byte[] ProcessBlockEncrypt(byte[] data, bool isFinalBLock, string padding)` - функция добавления блока открытого текста для зашифрования, содержит вызов функции `BlockCipherEncrypt`, где

`data` - блок для шифрования

`isFinalBLock` - флаг того, что передан последний блок шифруемого открытого текста

`padding` - вид дополнения, принимает значение `PKCS7` или `NON`.

возвращает зашифрованный блок данных.

В рамках данной функции должна быть реализована логика всех режимов, т.е. ветвление по режимам происходит тут.

Обрботка дополнения (добавление для последнего блока) должна происходить тут. Для `ProcessBlockDecrypt` должно происходить удаление дополнения для последнего блока.

#### **NB. А как тут использовать IV?**

Рассмотрим, как пример, режим CBC. Предположим, что значение, равно зашифрованию предыдущего блока помещается в виде массива байт
в переменную `byte[] last_block` (см. картинку в конце описания лабы). Явно IV в данном методе использовать не будем.  Вместо этого реализуем следующую логику в начале метода:

```
byte[] ProcessBlockEncrypt(byte[] data, bool isFinalBLock, string padding)
{
    # todo: необходимые проверки входов
    # считаем, что если `last_block` не задан, сдедовательно шифруем первый блок, генерируя IV
    IF last_block == NULL { last_block = GenRandomIv(); }
    # прочий код
}
```

При реализации метода расшифрования `ProcessBlockDecrypt` (см. ниже) при реализации формата шифртекста указанного в "**NB** Способ передачи значения" (см ниже) достаточно считать первый блок вектором инициализации (IV), возвращая при этом пустой массив при его передаче:

```
byte[] ProcessBlockDecrypt(byte[] data, bool isFinalBLock, string padding)
{
    # todo: необходимые проверки входов
    IF last_block == NULL { last_block = data; return EMPTY<byte[]>; }
    # прочий код
}
```

Похожий приём возможно использовать и для реализации других режимов.

#### **NB. Но... зачем `ProcessBlockEncrypt/ProcessBlockEncrypt`?**

Данные методы реализуют т.н. "поточный интерфейс", который позволяет шифровать данные поблочно, не загружая их полностью в память. Т.е. пользователю достаточно считать 1
блок данных в память, зашифровать его с помощью `ProcessBlockEncrypt`, получить блок шифртекста, записать его, считать второй блок данных для шифрования, и т.д.

### `byte[] BlockCipherEncrypt(byte[] data)` - функция шифрования блока данных алгоритмом AES, где

`data` - блок для шифрования

возвращает зашифрованный блок данных

В ходе реализации функции `BlockCipherEncrypt` необходимо пользоваться стандартной или общеизвестной реализацией алгоритма AES. Если библиотека не поддерживает одноблочное шифрование AES, необходимо воспользоваться режимом ECB.

Аналогично реализовать функции расшифрования `BlockCipherDecrypt` и `ProcessBlockDecrypt`.

При шифровании/расшифровании необходимо инициализировать объект блочного шифра только один раз для сообщения (не на каждый блок).

#### **NB. Но... зачем `BlockCipherEncrypt`?**

Для реализации режимов, нам нужна функция "одноблочного шифрования блочного шифра AES" (та самая "коробочка" с лекции, которая n бит в n бит отобразит). Почти всегда библиотеки не дают доступа к такой функции, но дают доступ к режиму ECB, который на одном блоке даёт как раз нужную нам функцию. Ровно это и надо сделать в `BlockCipherEncrypt`.

Пример одноблочного шифрования на C#.
```csharp
namespace AesExample
{
    using System;
    using System.Security.Cryptography;
    using System.Text;

    internal class Program
    {
        private static void Main(string[] args)
        {
            // Your Key here, 16 bytes
            byte[] key =
                { 0x73, 0x69, 0x78, 0x74, 0x65, 0x65, 0x6e, 0x2d,
                0x62, 0x79, 0x74, 0x65, 0x2d, 0x6b, 0x65, 0x79 };

            // Plaintext, 16 bytes string, utf-8
            string stringPt = "sixteen-byte-msg";

            // Raw Plaintext, bytes
            byte[] pt = Encoding.UTF8.GetBytes(stringPt);

            // Resulted Ciphertext will be here
            byte[] ct = new byte[16];

            // Create new AES instance
            using(Aes aes = new AesCryptoServiceProvider())
            {
                // Select Encryption mode
                aes.Mode = CipherMode.ECB;
                aes.Padding = PaddingMode.None;

                // Create encryptor with your key and zero IV
                var aesEncryptor = aes.CreateEncryptor(key, new byte[16]);
                
                // Transform one block
                ct = aesEncryptor.TransformFinalBlock(pt, 0, 16);

                // Get hex-string representation of Ciphertext
                string hex = BitConverter.ToString(ct);
                Console.WriteLine(hex.Replace("-", ""));
            }
        }
    }
}
```

c++
```cpp
#include <stdio.h> 
#include <openssl/aes.h>   

static const unsigned char key[] = {
    0x73, 0x69, 0x78, 0x74, 
    0x65, 0x65, 0x6e, 0x2d,
    0x62, 0x79, 0x74, 0x65,
    0x2d, 0x6b, 0x65, 0x79
};

int main()
{
    unsigned char text[]="sixteen-byte-msg";
    unsigned char enc_out[80];
    unsigned char dec_out[80];

    AES_KEY enc_key;

    AES_set_encrypt_key(key, 128, &enc_key);
    AES_encrypt(text, enc_out, &enc_key);      

    int i;

    for(i=0;*(enc_out+i)!=0x00;i++)
    {
       printf("%X ",*(enc_out+i));
    }
    printf("\n");

    return 0;
} 
```

python
```python
from Crypto.Cipher import AES
cipher = AES.new(b'sixteen-byte-key',AES.MODE_ECB)
cipher.encrypt(b'sixteen-byte-msg').hex()
```

## 2. Реализовать режимы ECB, CBC, CFB, OFB, CTR с использованием функции ` ProcessBlockEncrypt\ProcessBlockDecrypt`.

Реализовать интерфейс

### `byte[] Encrypt(byte[] data, byte[] iv = null)` - ф-я зашифрования на заданном ключе

### `byte[] Decrypt(byte[] data, byte[] iv = null)` - ф-я расшифрования на заданном ключе, где

`data` - массив байт для шифрования

`iv` - вектор инициализации или начальное заполнение счётчика в указанном режиме. Если значение не передано, или передано значение `null` (или пустой массив), но режим требует использования `IV` или счётчика - значение должно быть сгенерировано (через отдельный метод, с использованием системного криптографически стойкого генератора).

#### **NB.** `ProcessBlockEncrypt/ProcessBlockDecrypt` являются открытым api доспутным пользователю.
Результат итеративного вызова `ProcessBlockEncrypt/ProcessBlockDecrypt` на блоках данных должен соответствовать результату вызова `Encrypt/Decrypt`.

Для генерации ключей и `IV` использовать стойкие стандартные генераторы. В качестве дополнения (для режима CBC) использовать `PKCS7 padding` 
(дополнение, см [википедия](https://en.wikipedia.org/wiki/Padding_(cryptography)#PKCS%235_and_PKCS%237))

**IV должен быть случайным для режимов CBC, CFB, OFB.**

**Режимы CBC и ECB должен быть выполнен с использованием `PKCS7` дополнения, режимы  CFB, OFB, CTR - без паддинга (`NON`), путём отбрасывания лишних битов гаммы.**

**Выбор начального заполнения счётчика для режима CTR должен быть согласован с [rfc3686](https://tools.ietf.org/html/rfc3686#page-7).**

#### **NB.** Способ передачи значения `IV` обычно остаётся на усмотрение пользователя библиотеки. 
Иногда для этого используются специальные структуры данных (когда необходимо помимо `IV` передать некоторые метаданные о шифровании (например режим и выбранный шифр), в которые помещается `IV` в специальное для него поле. Если же необходимо передавать только шифртекст - `IV` часто просто конкатенируется в его начало:

`c = IV || E(k, m)`

При расшифровании получатель знает длину `IV` (размер блока) и может разбить `c` на `IV` и шифртекст.

#### **NB.** `IV` не должен быть секретным при передаче по каналу связи, единственные требования на него - случайность (равномерное распределение) и уникальность для каждого шифруемого сообщения.
Отсюда - при каждом шифровании одного и того же открытого текста мы получим новый шифртекст, так как используется новый случайный `IV`.

## 2.5 Использовать реализации режима CBC в вашем языке программирования для валидации вашей реализации режима CBC.

Например зашифровать с помощью существующей реализации и расшифровать с помощью кода лабы, и наоборот.

## 3. Расшифровать следующие шифртексты:

Режим CBC и CTR. IV = 16 байт. IV добавлен к зашифрованному тексту в начале. CBC: PKCS7 padding, ascii


    CBC key: 140b41b22a29beb4061bda66b6747e14
    CBC Ciphertext 1: 4ca00ff4c898d61e1edbf1800618fb2828a226d160dad07883d04e008a7897ee2e4b7465d5290d0c0e6c6822236e1daafb94ffe0c5da05d9476be028ad7c1d81
    
    CBC key: 140b41b22a29beb4061bda66b6747e14
    CBC Ciphertext 2: 5b68629feb8606f9a6667670b75b38a5b4832d0f26e1ab7da33249de7d4afc48e713ac646ace36e872ad5fb8a512428a6e21364b0c374df45503473c5242a253
    
    CTR key: 36f18357be4dbd77f050515c73fcf9f2
    CTR Ciphertext 1: 69dda8455c7dd4254bf353b773304eec0ec7702330098ce7f7520d1cbbb20fc388d1b0adb5054dbd7370849dbf0b88d393f252e764f1f5f7ad97ef79d59ce29f5f51eeca32eabedd9afa9329
    
    CTR key: 36f18357be4dbd77f050515c73fcf9f2
    CTR Ciphertext 2: 770b80259ec33beb2561358a9f2dc617e46218c0a53cbeca695ae45faa8952aa0e311bde9d4e01726d3184c34451
    
## 4. Для каждого режима шифрования зашифровать и расшифровать произвольный текст длины 2,5 блока.



## PS. Небольшие спойлеры по лабе в части CTR, если очень сложно читать rfc

 * `nonce` - уникально в рамках одного соединения

 * `IV` уникально для каждого сообщения

 * `Block Counter` уникальность для каждого блока

```
The encryptor can generate the IV in any
manner that ensures uniqueness. Common approaches to IV generation
include incrementing a counter for each packet and linear feedback
shift registers (LFSRs).
```

 * если для `IV` и `counter` использовать счётчики - всё будет хорошо в плане безопасности

 * `nonce` нужно генерить случано, если следовать rfc, но один раз для одного соединения

 * пара `nonce`-`iv` действительно должна быть уникальной для всех сообщений при фиксированном ключе

При расшифровании разбивать счётчик на части не нужно. Можно трактовать первые 16 байт шт как начальное заполнение счётчика, которое используется для выработки гаммы для первого блока от. Для шифрования последующих блоков - начальное заполнение инкрементируется нужное число раз перед зашифрованием с целью получения гаммы для соответствующих блоков от.

Небольшой фрагмент псевдокода, показывающего эволюцию заполнения блока счётчика `CTR` при шифровании сообщения:

```
CTR = NONCE || CTR_MSG || CTR_BLOCK

let CTR = 1234 || 0000 || 0000

let m = m_0 || m_1 || m_2 || m_3

// CTR = 1234 || 0000 || 0000
Enc(m_0) -> { c_0 = E(CTR) + m_0; INC_BLOCK(CTR); }
// CTR = 1234 || 0000 || 0001
Enc(m_1) -> { c_1 = E(CTR) + m_1; INC_BLOCK(CTR); }
// CTR = 1234 || 0000 || 0002
Enc(m_2) -> { c_3 = E(CTR) + m_2; INC_BLOCK(CTR); }
// CTR = 1234 || 0000 || 0003
Enc(m_3, FINAL) -> { c_3 = E(CTR) + m_3; INC_MSG(CTR); }
// CTR = 1234 || 0001 || 0000

let INC_BLOCK(CTR as X || Y || Z) -> { CTR = (X || Y || Z +1 }
let INC_MSG(CTR as X || Y || Z) -> { CTR = (X || Y+1 || 0* }
```

## PS. Небольшие спойлеры по лабе в части дополнения, если очень сложно читать wiki
Дополнение (падинг) всегда добавляется и всегда снимается для режимов, для которых он необходим. Зачем - рассмотрим на примере.
Пусть размер блока 4 "символа". Имеем 2 сообщения
```
m_1 = abc
m_2 = abc1
```

Если мы будем использовать дополнение, только для первого сообщения, а для второго не будем - то они обо будут иметь вид
```
abc1
```
и мы не сможем при расшифровании понять, надо ли убирать 1.

Выход такой - использовать дополнение всегда, даже если блок уже выравнен по длине - в таком случае добавим целый блок дополнения
```
E(m_1) = E(pad(m_1)) = E(abc1) = c_1
m_1 = unpad(D(c_1)) = unpad(abc1) = abc

E(m_2) = E(pad(m_2)) = E(abc1 | 4444) = c_2
m_2 = unpad(D(c_2)) = unpad(abc1 | 4444) = abc1

m_2 != m_1
```

## PS. Особенности реализций режимов OFB и CFB в ряде библиотек
Формально режимы OFB и CFB могут быть определены в более общем виде, где в качестве обратной связи подаётся только часть бит (см. NIST SP 800-38A и ГОСТ Р 34.13-2015). 
В рамках лабораторной работы необходимо использовать полный блок в качестве обратной связи. Если хочется проверить собственную реализацию на совместимость - рекомендуется 
поискать в библиотечной реализации параметр, задающий размер обратной связи (например, `segment_size` для [python](https://pycryptodome.readthedocs.io/en/latest/src/cipher/aes.html)).
В зависимости от реализации и языка параметр может задавать размер как в байтах, так и в битах.

Подробнее:

https://stackoverflow.com/questions/63172967/how-to-set-aes-cfb-feedback-segment-size-in-nodejs-crypto-lib

https://security.stackexchange.com/questions/85727/what-is-the-segment-size-when-using-cipher-feedback-cfb-chaining-mode

https://stackoverflow.com/questions/59712935/pycryptodome-cryptography-inequality-with-aes-cfb-in-python

https://csrc.nist.gov/pubs/sp/800/38/a/final

https://tc26.ru/standard/gost/GOST_R_3413-2015.pdf

## Чеклист лабораторной работы.
- реализован поточный интерфейс `ProcessBlockEncrypt/ProcessBlockDecrypt`, результат его работы соответствует методу `Encrypt/Decrypt`
- поточные режимы шифрования (OFB, CFB, CTR) корректно работают без добавления дополнения
- корректная проверка режимов, с возвратом ошибки или исключения при неподдерживаемом режиме, текстовка ошибки включает в себя имя неподдерживаемого режима (пример `raise ValueError(а"wrong mode [{self.mode}] detected")`)
- обработка некорректных данных сделана через коды ошибок или исключения (никаких assert!)
- ключ и/или объект шифра не создаётся на каждой итерации
- обработка дополнения производится в методах поточного интерфейса `ProcessBlockEncrypt/ProcessBlockDecrypt`
- в случае добавления нового блока дополнения в `ProcessBlockEncrypt` данная функция должна вернуть 2 блока шт, корректно зашифрованных в указанном режиме.
`ProcessBlockDecrypt` при расшифровании целого блока дополнения должна возвращать пустой массив.
- начальное заполнение счётчика в режиме CTR при зашифровании выбрано в соответствии с RFC (не забыть корректно обновлять счётчик на каждый блок и на каждое шифрование нового ОТ(если поддерживается)).
При расшифровании разбивать счётчик на "кусочки" не нужно, достаточно взять весь блок как начальное заполнение счётчика и инкрементировать его целиком.
- не забыть про пункт 2.5

Страшная непонятная картинка с лекции (CBC шифрование):
![изображение](https://user-images.githubusercontent.com/15075638/216041949-94dba95d-e8ee-49c1-99f3-baafc6d64f89.png)

